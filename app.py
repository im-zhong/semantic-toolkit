...
# Streamlit 页面固定配置：不允许用户通过页面修改
APP_FIXED_MODEL = "glm-4-flash"
APP_FIXED_TOP_K = 80
APP_FIXED_JSON_PATH = DEFAULT_JSON_PATH
...
def get_classifier() -> CLCAutoClassifier:
    return CLCAutoClassifier(
        json_path=APP_FIXED_JSON_PATH,
        model=APP_FIXED_MODEL,
        top_k=APP_FIXED_TOP_K,
    )
...
with st.sidebar:
    st.header("系统配置")
    st.caption("以下配置由系统固定，用户不能修改。")

    st.write("中图分类法 JSON 路径")
    st.code(APP_FIXED_JSON_PATH)

    st.write("智谱模型名称")
    st.code(APP_FIXED_MODEL)

    st.write("本地候选召回数量")
    st.code(str(APP_FIXED_TOP_K))
